...
class LinRegGA(GeneticAlgorithm):
    ...
    def on_fitness(self):
        ...
        self.objective = [
            self.lr.get_fitness(self.lr.get_columns(self.data, bitstring), self.values) for bitstring in self.population
        ]
    ...
    def plot_entropy(self):
        ...
        with Path("data/linreg_crowding.p").open(mode="rb") as fp:
            crowding = pickle.load(fp)
        with Path("data/linreg_no_crowding.p").open(mode="rb") as fp:
    ...
    def save_entropy(self):
        ...
        if self._crowding:
            with Path("data/linreg_crowding.p").open(mode="wb") as fp:
                pickle.dump(self._entropy, fp)
        else:
            with Path("data/linreg_no_crowding.p").open(mode="wb") as fp:
                pickle.dump(self._entropy, fp)
